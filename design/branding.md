...
| **computatio** | a reckoning, a calculation | everything `design/computation.md` covers: how an expression is computed, interpreted or compiled, and bounded. Proposed with its parts named by how they run: **interpretatio** (walking the expression tree: symbolic and numeric evaluation alike), **compilatio** (lowering to code), and **evaluatio** or **aestimatio** for numeric evaluation. The catch: the code and `AGENTS.md` already say "evaluate" for the symbolic step (`.evaluate()`, "evaluates to") and `N` for the numeric one, so an **evaluatio** that means numeric work reads backwards against both. Overlaps the nucleus reading above. |
...
- `design/computation.md` describes how we compute — symbolic and numeric evaluation,
  compilation — in plain words, and documents that package in its §5.
