...
### Name collision to settle first

`<Head>.yaml` already has a head-level `implementations:` list, used by 54 records. It holds definitions, the TS source, and the systems the head maps to. That is a different thing from `<Head>.implementations.yaml`, which holds per-example runs. Options:

1. Rename the file, e.g. to `<Head>.runs.yaml`. "Implementations" then means only the head-level list.
2. Rename the field, e.g. to `realisations:`, and keep the file name.
3. Keep both names, and say "the implementations record" for the file and "the implementations list" for the field.

I lean toward 1: the file really is a record of runs, and an implementation is what produces a run. But we just settled on `.implementations.yaml`, so this is your call.
...
| `oracle/src/mappings.ts` `MAPPINGS`                                                                   | ~86 (24 without a record)   | head-level `implementations:` rows with `origin: external`, one row per system: `form: sympy`, `template: "primepi($1)"`, plus `arity`, `threadArg` and `note` |
...
MAPPINGS is the move that pays off most. The page already shows the head-level `implementations:` list as "the systems it maps to", and MAPPINGS is a second hand-kept copy of the same fact.
...
2. Resolve the name collision above, if you pick option 1 or 2.
3. Move `curated.ts` into `references:` and `names:`. Its only consumer is the crosswalk, and `check-crosswalk.ts` already fetches every URL.
4. Move `RENAMED` into `formerly:`.
5. Move the Wolfram `HEADS` into `names.wolfram`. The test: `toWolfram` gives the same output for every key of `HEADS` across the round-trip golden set.
6. Move `MAPPINGS` into `implementations:` rows. The test: `emit` gives the same source for every example on every system, and the nightly drift check stays quiet across the switch.
7. Replace the declare descriptions with `summary`.
8. Delete `migrate/`.
