...
That is why `packages/elements/src/traditional.ts` exists at all: an output-only side table
...
the field (`packages/elements/src/source.ts`); `Gamma` → `\Gamma` is right, this one is
not.
