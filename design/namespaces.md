...
- The one-time dump (`scripts/extract.ts` folding an enumeratio database dump into
  `src/catalog-data.ts`) is retired: what `declareCatalog` needs lives on each head's own
  record instead (`grades`/`carrier`/`unbounded`, `catalogCarrier`, `statOn`/`mapOn`), and
  `src/catalog-records-data.ts` (generated by `packages/reference/scripts/collect-catalog-
records.ts`, pinned by a currency test) is what `resources.ts` reads.
