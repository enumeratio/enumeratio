...
`RandomElement`/`RandomSample` draw through the library's O(1) `at`; `RandomShuffle` (CE's own head) permutes a
list or any collection. The **reshuffle** button (bottom-right) rerolls them all; it is disabled when a notebook
has no randomness.
...
<enumeratio-notebook value='{"lines":[
  ...
  {"id":"d3","latex":"RandomShuffle(\\left\\lbrack1,2,3,4,5\\right\\rbrack)"}
]}'></enumeratio-notebook>
</ClientOnly>
