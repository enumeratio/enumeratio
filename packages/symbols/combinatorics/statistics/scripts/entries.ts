// The statistics package's generated reference entries — one per declared statistic, plus
// one per frontier signature. The definitions already carry a head, a carrier and a summary;
// what they cannot carry is a worked example, so this evaluates each definition against a
// couple of fixed inputs per carrier and pins the results.
//
...
// scripts/collect-entries.ts writes them; tests/generated.test.ts checks they're current.
// The rest of reference/ (the distributions) is hand-written and not ours to touch.
...
const entryFor = (definition: Definition): ReferenceEntry => {
  ...
  return {
    ...
    domain: DOMAIN[definition.on] ?? definition.on,
    signature: `${definition.head}(${SUBJECT_NAME[definition.on] ?? "_"})`,
...
const frontierEntryFor = (frontier: (typeof FRONTIER)[number]): ReferenceEntry => {
  ...
  return {
    name: frontier.head,
    domain: DOMAIN[frontier.on] ?? frontier.on,
    signature: `${frontier.head}(${SUBJECT_NAME[frontier.on] ?? "_"})`,
    summary: `${frontier.why} Not yet defined.`,
    details: [
...
/** One entry per DECLARED head (a head defined on two carriers is declared once), plus one per
 * frontier signature, whose single example is aspirational: the head has no definition, so
 * the example records what it would answer and the test asserts the gap is still open. */
export const generated: GeneratedEntries = {
  entries: [...[...owner.values()].map(entryFor), ...FRONTIER.map(frontierEntryFor)],
  // A core-owned head is ours too: if compute-engine starts declaring one, its record goes.
  owned: new Set([...ALL_STATISTICS.map((d) => d.head), ...FRONTIER.map((f) => f.head)]),
  // Anything else on a record (`formerly`, `references`, `names`) is curated by hand.
  fields: new Set(["name", "domain", "signature", "summary", "details", "examples"]),
};
