...
export function IsMultisetOf(m: number[], n: number, k: number): boolean {
  ...
  let prev = 1;
  for (const x of m) {
    if (!Number.isInteger(x) || x < prev || x > n) return false;
    prev = x;
  }
...
function pruferEncode(edges: number[][], n: number): number[] {
  ...
  const removed = Array.from({ length: n + 1 }, () => false);
  const seq: number[] = [];
  for (let i = 0; i < n - 2; i++) {
    let leaf = -1;
    for (let v = 1; v <= n; v++)
...
export function InvolutionUnrank(n: number, rank: number): number[] {
  ...
  const rec = (labels: number[], r: number): void => {
    ...
    result[partner] = last;
    rec(
      labels.filter((x) => x !== last && x !== partner),
      r % block,
    );
  };
...
export function MotzkinRank(path: number[]): number {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (step === 1) {
      ...
      r += motzkinCompletions(s - 1, h + 1);
    } else {
      r += motzkinCompletions(s - 1, h + 1) + motzkinCompletions(s - 1, h);
      h--;
    }
  }
  return r;
...
export function FibonacciWordUnrank(n: number, rank: number): number[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    r -= zero;
    out.push(1);
    last = 1; // place a 1 (only allowed if last !== 1, guaranteed by the count split)
    void last;
  }
  return out;
}
...
export function IsPartitionInBox(p: number[], a: number, b: number): boolean {
  ...
  let prev = Infinity;
  for (const x of p) {
    if (!Number.isInteger(x) || x < 1 || x > b || x > prev) return false;
    prev = x;
  }
  return true;
}
...
function schroderCompletions(w: number, h: number): number {
  ...
  if (v === undefined) {
    v =
      schroderCompletions(w - 1, h + 1) +
      (h > 0 ? schroderCompletions(w - 1, h - 1) : 0) +
      (w >= 2 ? schroderCompletions(w - 2, h) : 0);
    _schrMemo.set(key, v);
  }
  return v;
}
export function SchroederCount(n: number): number {
...
export function SchroederRank(tokens: number[]): number {
  let r = 0,
    w = 0,
    h = 0;
...
export function IsSchroederPath(tokens: number[], n: number): boolean {
...
function dyckToForest(path: number[]): { forest: OrdTree[]; pos: number } {
  ...
  const build = (): OrdTree => {
    ...
    while (pos < path.length && path[pos] === 1) {
      pos++;
      children.push(build());
      pos++; /* matching 0 */
    }
...
function ordTreeEdges(t: unknown): number {
...
export function IsOrderedTree(t: unknown, n: number): boolean {
  const wellFormed = (x: unknown): boolean => Array.isArray(x) && x.every(wellFormed);
...
export type KTree = 0 | KTree[];
const _fillMemo = new Map<string, number>();
function karyTreeCount0(s: number, k: number): number {
  return s === 0 ? 1 : cntFill(s - 1, k, k);
}
...
function kSize(t: KTree): number {
  if (t === 0) return 0;
  let s = 0;
  for (const c of t) s += kSize(c);
  return 1 + s;
}
function unrankForest(m: number, slots: number, k: number, r: number): KTree[] {
  if (slots === 0) return [];
  for (let s = 0; s <= m; s++) {
...
function unrankKTree(n: number, k: number, r: number): KTree {
  if (n === 0) return 0;
...
function rankForest(children: KTree[], k: number): number {
  let r = 0;
  let m = 0;
  for (const c of children) m += kSize(c);
...
function rankKTree(t: KTree, k: number): number {
  return t === 0 ? 0 : rankForest(t, k);
}
export function KAryTreeRank(t: KTree, k: number): number {
...
import { CountSurjections } from "./kernels-combinatorics.ts";
...
function surjCompletions(remaining: number, missing: number, k: number): number {
  ...
  if (v === undefined) {
    v =
      missing * surjCompletions(remaining - 1, missing - 1, k) +
      (k - missing) * surjCompletions(remaining - 1, missing, k);
    _surjMemo.set(key, v);
  }
  return v;
}
...
export function IsSurjectionOf(word: number[], n: number, k: number): boolean {
  if (!Array.isArray(word) || word.length !== n) return false;
  const hit = Array.from({ length: k + 1 }, () => false);
  let cnt = 0;
  for (const x of word) {
    if (!Number.isInteger(x) || x < 1 || x > k) return false;
    if (!hit[x]) {
      hit[x] = true;
      cnt++;
    }
...
export function BinaryStringCount(n: number): number {
  return 2 ** n;
}
...
export function CyclicPermutationCount(n: number): number {
...
export function CyclicPermutationUnrank(n: number, rank: number): number[] {
  ...
  const image: number[] = Array.from({ length: n }, () => 0);
...
function doubleFactOdd(k: number): number {
  let f = 1;
  for (let i = k; i >= 1; i -= 2) f *= i;
  return f;
}
export function PerfectMatchingCount(n: number): number {
  return n < 0 ? 0 : doubleFactOdd(2 * n - 1);
}
...
export function PerfectMatchingRank(pairs: number[][], n: number): number {
  // partner lookup
  const partner = Array.from({ length: 2 * n + 1 }, () => 0);
...
export function IsPerfectMatchingOf(pairs: unknown, n: number): boolean {
  if (!Array.isArray(pairs) || pairs.length !== n) return false;
  const seen = Array.from({ length: 2 * n + 1 }, () => false);
  for (const e of pairs) {
    if (!Array.isArray(e) || e.length !== 2) return false;
    for (const x of e) {
      if (!Number.isInteger(x) || x < 1 || x > 2 * n || seen[x]) return false;
      seen[x] = true;
    }
  }
  return true;
...
function partsLeq(m: number, j: number): number {
  ...
  let v = _pamX.get(key);
  if (v === undefined) {
    v = partsLeq(m, j - 1) + partsLeq(m - j, j);
    _pamX.set(key, v);
  }
  return v;
}
export function PartitionsMaxPartCount(n: number, m: number): number {
  return n < 0 ? 0 : partsLeq(n, m);
}
...
export function IsPartitionMaxPart(p: number[], n: number, m: number): boolean {
  ...
  let s = 0,
    prev = Infinity;
  for (const x of p) {
    if (!Number.isInteger(x) || x < 1 || x > m || x > prev) return false;
    s += x;
    prev = x;
  }
  return s === n;
...
export function RootedForestUnrank(n: number, rank: number): number[] {
  ...
  const seen = Array.from({ length: n + 2 }, () => false);
  const queue = [root];
  seen[root] = true;
  while (queue.length) {
...
export function RootedForestRank(parent: number[], n: number): number {
  ...
  for (let i = 1; i <= n; i++) {
    const p = parent[i - 1];
    edges.push([i, p === 0 ? n + 1 : p]);
  }
  return LabeledTreeRank(edges, n + 1);
}
export function IsRootedForest(parent: unknown, n: number): boolean {
...
export function DigitCountKernel(n: number, base = 10): number[] {
  const b = base >= 2 ? Math.trunc(base) : 10;
  const cnt = Array.from({ length: b }, () => 0);
