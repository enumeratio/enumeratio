...
test("VertexInDegree + VertexOutDegree relate to VertexDegree the same way on directed vs. undirected edges", () => {
  const g = graph([U(1, 2), D(2, 3), D(3, 2)]);
  // vertex 2: undirected(1,2) contributes to both in and out; D(2,3) is out-only; D(3,2) is in-only.
  expect(run(["VertexInDegree", g, 2])).toBe(2); // U(1,2) + D(3,2)
  expect(run(["VertexOutDegree", g, 2])).toBe(2); // U(1,2) + D(2,3)
  const deg = run(["VertexDegree", g, 2]) as number;
  expect(deg).toBe(3); // one endpoint-count per incident edge, direction-blind
});
...
test("EigenvectorCentrality of a path's middle vertex exceeds its ends (P3)", () => {
  const g = graph([U(1, 2), U(2, 3)]);
  const c = run(["EigenvectorCentrality", g]) as unknown as [string, ...number[]];
  const [, c1, c2, c3] = c;
  expect(c2 as number).toBeGreaterThan(c1 as number);
  expect(c2 as number).toBeGreaterThan(c3 as number);
  expect(c2).toBe(1); // normalized so the max entry is 1
});
