...
test("Fibonacci and LucasL on Ẑ", () => {
  ...
  expect(run(["Fibonacci", profinite(["Rational", 1, 2], 3)])).toEqual(["Fibonacci", profinite(["Rational", 1, 2], 3)]);
});
...
test("AdicNumeral(p, z) projects to Q_p", () => {
  ...
  expect(
    run(["ProfiniteNumber", ["List", ["AdicNumeral", 2, profinite(100, 24)], ["AdicNumeral", 3, profinite(100, 24)]]]),
  ).toEqual(profinite(4, 24));
});

test("adèles: componentwise, rationals on the diagonal", () => {
  expect(run(["Adele", 5])).toEqual(["Adele", 5, 5]);
  expect(run(["Multiply", ["Adele", 2, profinite(1, 6)], 3])).toEqual(["Adele", 6, profinite(3, 18)]);
  ...
  expect(run(["Equal", ["Adele", 2, profinite(2, 6)], ["Adele", 1, profinite(5, 9)]])).toBe("False");
...
test("idèles: principal, local, a listed component carries its valuation into the scale", () => {
  ...
  expect(run(["Idele", 1, 1, ["List", ["AdicNumeral", 2, 3, 1]]])).toEqual(["Idele", 1, 1, ["List"]]);
...
test("ProfinitePlot draws the identity as a diagonal", () => {
  ...
  expect(rows).toEqual(Array.from({ length: 6 }, (_, i) => Array.from({ length: 6 }, (_, j) => (i + j === 5 ? 1 : 0))));
