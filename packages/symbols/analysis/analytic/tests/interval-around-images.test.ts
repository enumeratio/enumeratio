...
test("Interval: hyperbolic, Exp, Ln and Sqrt images", () => {
  ...
  expect(json(["Exp", ["Interval", -1, ["Ln", 2]]])).toEqual(["Interval", ["Divide", 1, "ExponentialE"], 2]);
...
test("Interval: Sin/Cos over an unbounded interval is the oscillation range [-1, 1]", () => {
  expect(json(["Sin", ["Interval", "NegativeInfinity", "PositiveInfinity"]])).toEqual(["Interval", -1, 1]);
});
...
test("Around: propagation across trig, inverse-trig, hyperbolic and log heads", () => {
  expect(json(["Sin", ["Around", 2, 0.01]])).toEqual(["Around", 0.9092974268256817, 0.004161468365471424]);
  expect(json(["Cot", ["Around", 3, 0.01]])).toEqual(["Around", -7.015252551434534, 0.5021376836040873]);
...
test("Around: propagation across Γ, Erf/Erfc/ErfInv, and multi-argument heads at a fixed position", () => {
  ...
  expect(json(["Erfc", ["Around", 2, 0.01]])).toEqual(["Around", 0.004677734981047266, 0.00020666985354092054]);
  ...
  const harmonic = json(["HarmonicNumber", ["Rational", 3, 2], ["Around", 2.1, 0.01]]) as [string, number, number];
  ...
  expect(json(["HurwitzZeta", 2, ["Around", 0.5, 0.01]])).toEqual(["Around", 4.934802200544679, 0.1682879664423432]);
});
...
test("N(): the E-base Power rule still fires once N() has decimalized the base (issue #113 fix)", () => {
  ...
  expect(ce.box(["N", ["Exp", ["Around", 0, 0.1]]] as never).evaluate().json).toEqual(["Around", 1, 0.1]);
});
