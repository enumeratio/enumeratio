...
const agrees = (ours: BigDecimal | undefined, mpmath: string): void => {
  expect(ours?.toPrecision(COMPARED).toString()).toBe(
    new BigDecimal(mpmath).toPrecision(COMPARED).toString(),
  );
};
...
test("LerchPhi: inside the disk, a growing-then-shrinking series, and outside it", () => {
  at(() => {
    ...
    expect(
      lerchPhiBig(new BigDecimal(2), new BigDecimal(2), BigDecimal.ONE, DIGITS),
    ).toBeUndefined();
  });
});
...
test("BarnesG: near 1, and reached by the recurrence up and down", () => {
  at(() => {
    agrees(
      barnesGBig(new BigDecimal("0.5"), DIGITS),
      "0.603244281209446206191429224534702079883003420389459765387769",
    );
    agrees(
      barnesGBig(new BigDecimal("7.3"), DIGITS),
      "204559.791949272870551283619621883605369086814928488982777458",
    );
    agrees(
...
test("StieltjesGamma: the classical constants and a shifted one", () => {
  at(() => {
    ...
    agrees(
      stieltjesGammaBig(2, third(), DIGITS),
      "3.61916390961896362988718221618594170329408335142691980717624",
    );
  });
});
