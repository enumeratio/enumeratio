...
test("SeriesCoefficient: ordinary-point Taylor coefficients, matching wolframscript", () => {
  ...
  expect(evalJson(["SeriesCoefficient", ["Sin", "x"], ["List", "x", 0, 5]])).toEqual([
    "Rational",
    1,
    120,
  ]); // wolframscript: 1/120
  expect(
    evalJson(["SeriesCoefficient", ["Divide", 1, ["Subtract", 1, "x"]], ["List", "x", 0, 10]]),
  ).toEqual(1); // wolframscript: 1
  expect(evalJson(["SeriesCoefficient", ["Ln", ["Add", 1, "x"]], ["List", "x", 0, 4]])).toEqual([
    "Rational",
    -1,
    4,
  ]); // wolframscript: -1/4
...
test("Assuming scopes ce.assume for the duration of the call, and never leaks", () => {
  ...
  expect(
    evalJson(["List", ["Assuming", ["Greater", "z", 0], ["Sign", "z"]], ["Sign", "z"]]),
  ).toEqual(["List", 1, ["Sign", "z"]]);
});
...
test("Refine simplifies under a stated (or already-assumed) condition", () => {
  ...
  expect(evalJson(["Refine", ["Sqrt", ["Power", "x", 2]], ["Less", "x", 0]])).toEqual([
    "Negate",
    "x",
  ]);
  ...
  expect(evalJson(["Refine", ["Greater", ["Power", "x", 2], 0]])).toEqual([
    "Less",
    0,
    ["Power", "x", 2],
  ]);
