...
type Json = ReturnType<typeof parseNotatio>["json"];
...
/**
 * Every attribute/property under `root` that is a notatio expression over `names`.
 *
...
export function captureTemplates(
  ...
  const slotted = (src: string): Json | undefined => {
    ...
    const { json, wildcards, errors } = parseNotatio(src, { parseLatex });
 ...
 * evaluate, and write the re-serialized notatio back. A template that fails to
...
export function applyTemplates(
  ...
  for (const t of templates) {
    let next: string;
    try {
      next = serializeNotatio(engine.box(t.json).subs(subs).evaluate().json);
    } catch {
      continue;
    }
    if ("attr" in t) {
