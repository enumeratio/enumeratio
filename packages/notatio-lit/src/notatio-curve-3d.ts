...
export class NotatioCurve3D extends LitElement {
  ...
  declare points: readonly Triple[] | undefined;
 ...
 */
export function pointsOf(value: BoxedExpression): Triple[] {
  if (value.operator === "List") {
    ...
    for (const row of rows) {
      ...
      const p: Triple = [a?.re ?? Number.NaN, b?.re ?? Number.NaN, c?.re ?? Number.NaN];
