...
describe("Product — Cartesian product with O(1) mixed-radix indexing", () => {
  it("count is the product; at is a pair", () => {
    expect(num(["Length", ["Product", ["Permutations", 3], ["Subsets", 2]]])).toBe(24); // 6·4
    expect(str(["At", ["Product", ["Permutations", 3], ["Subsets", 2]], 1])).toBe("[[1,2,3],[]]");
    // big product, random access stays instant
    expect(num(["Length", ["Product", ["Permutations", 10], ["Subsets", 20]]])).toBe(3628800 * 1048576);
  });
  it("Rank inverts At through the product", () => {
    const coll: any = ["Product", ["Permutations", 3], ["KSubsets", 4, 2]];
  ...
  it("membership requires both components", () => {
    const coll: any = ["Product", ["Permutations", 3], ["Subsets", 2]];
...
describe("Concat — two collections end to end", () => {
  it("count adds; at routes across the seam", () => {
    const coll: any = ["Concat", ["Subsets", 2], ["Permutations", 3]];
    expect(num(["Length", coll])).toBe(4 + 6);
    expect(str(["At", coll, 4])).toBe("[1,2]"); // last of Subsets(2)
    expect(str(["At", coll, 5])).toBe("[1,2,3]"); // first of Permutations(3)
...
describe("Window — O(1) slice (Take/Drop/Slice without materializing)", () => {
  it("start/len subrange over a huge collection", () => {
    expect(num(["Length", ["Window", ["Permutations", 12], 100, 5]])).toBe(5);
    expect(str(["At", ["Window", ["Permutations", 4], 3, 2], 1])).toBe(str(["At", ["Permutations", 4], 3]));
    expect(str(["At", ["Window", ["Permutations", 4], 3, 2], 2])).toBe(str(["At", ["Permutations", 4], 4]));
    // window clamps to the tail
    expect(num(["Length", ["Window", ["Permutations", 4], 23, 10]])).toBe(2);
  });
});
...
describe("Power — k-fold Cartesian power", () => {
  it("count is base^k; element is a k-tuple; Rank inverts At", () => {
    ...
    const coll: any = ["Power", ["Permutations", 3], 2];
    const N = num(["Length", coll]); // 36
    expect(N).toBe(36);
    for (let i = 1; i <= N; i++) expect(num(["Rank", coll, ce.box(["At", coll, i]).evaluate()])).toBe(i);
  });
  it("big power stays instant", () => {
    expect(num(["Length", ["Power", ["Permutations", 5], 6]])).toBe(120 ** 6);
  });
});

describe("Zip — parallel pairing", () => {
  it("count is the min; element is a pair; Rank inverts At", () => {
    expect(num(["Length", ["Zip", ["Permutations", 3], ["Subsets", 3]]])).toBe(6); // min(6,8)
    const coll: any = ["Zip", ["Permutations", 3], ["Subsets", 4]];
    const N = num(["Length", coll]);
    for (let i = 1; i <= N; i++) expect(num(["Rank", coll, ce.box(["At", coll, i]).evaluate()])).toBe(i);
  });
});
...
describe("Rank composes through views", () => {
  it("Reversed / Rotated", () => {
    expect(num(["Rank", ["Reversed", ["Permutations", 4]], ["List", 4, 3, 2, 1]])).toBe(1); // last→first
    expect(num(["Rank", ["Reversed", ["Permutations", 4]], ["List", 1, 2, 3, 4]])).toBe(24);
    const rot: any = ["Rotated", ["Permutations", 4], 1];
  ...
  it("nested: Rank through Reversed(Product(...))", () => {
    const coll: any = ["Reversed", ["Product", ["Permutations", 3], ["Subsets", 2]]];
    const N = num(["Length", coll]);
    for (let i = 1; i <= N; i++) expect(num(["Rank", coll, ce.box(["At", coll, i]).evaluate()])).toBe(i);
  });
});
