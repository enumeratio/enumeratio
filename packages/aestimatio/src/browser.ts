...
export interface BrowserEvaluateSessionOptions {
  /** On the dedicated-`Worker` fallback, hard-kills the worker after this many
   * milliseconds -- see `BrowserSession`'s own comment on what that does to state. On a
   * `SharedWorker`, there is no safe way to kill a context other tabs may be using, so
   * this only makes THIS call give up locally; the shared engine keeps running whatever
   * it was given. */
  readonly timeMs?: number;
...
export interface BrowserSessionEvaluateResult {
  ...
  /** True only on the dedicated-`Worker` fallback, exactly when THIS call's `timeMs`
   * killed the worker: bindings made before it are gone, and a fresh worker (with a
   * fresh engine) now backs the session. Always `false` on a `SharedWorker` session --
   * see `timeMs`'s own comment. */
...
export function openSession(options: BrowserSessionOptions = {}): BrowserSession {
  ...
  let killable: (() => void) | undefined; // undefined on a SharedWorker: not this tab's to kill
  ...
  if (sharedFactory !== undefined) {
    ...
    const createWorker = options.createWorker ?? globalWorkerFactory();
    let worker = createWorker(url, { type: "module" });
    port = worker as unknown as MessagePortLike;
    killable = () => worker.terminate();
    respawn = () => {
      worker = createWorker(url, { type: "module" });
      port = worker as unknown as MessagePortLike;
      killable = () => worker.terminate();
    };
  }
  // Every connection (a SharedWorker's per-tab port, or a dedicated worker acting as its
  // own one) starts with a handshake message carrying `setup` -- see
    ...
    return new Promise((resolve, reject) => {
      ...
      if (timeMs !== undefined) {
        timer = setTimeout(() => {
          ...
          signal?.removeEventListener("abort", onAbort);
          if (currentPort.onmessage === onMessage) currentPort.onmessage = null;
          if (killable !== undefined && respawn !== undefined) {
            // Dedicated-worker fallback: kill it outright (the only reliable cancel --
            ...
            // SharedWorker: nothing here is safe to kill on other tabs' behalf. This call
            // simply stops waiting -- the engine may still be running the input.
            resolve({ value: ABORTED, reset: false });
          }
        }, timeMs);
      }

      currentPort.postMessage({ id, json });
    });
  }
