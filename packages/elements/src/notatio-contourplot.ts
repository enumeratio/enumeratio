...
/**
 * `<notatio-contourplot expr="x^2 - y^2" xrange="-3,3" yrange="-3,3">` -- the
...
export class NotatioContourPlot extends LitElement {
  ...
  async #recompute(): Promise<void> {
    ...
    try {
      ...
      // Precompile to a native JS function -- see notatio-plot3d.ts's rationale.
  ...
  protected override render(): unknown {
    return html`<span class="notatio-contourplot-box">${unsafeHTML(this._svg)}</span>`;
  }
}

if (!customElements.get("notatio-contourplot")) {
  customElements.define("notatio-contourplot", NotatioContourPlot);
}
