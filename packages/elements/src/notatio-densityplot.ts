...
import { densitySvg } from "./densityplot.ts";
...
/**
 * `<notatio-densityplot expr="\sin(x)\cos(y)" xrange="-3,3" yrange="-3,3">` --
...
export class NotatioDensityPlot extends LitElement {
  ...
  protected override render(): unknown {
    return html`<span class="notatio-densityplot-box">${unsafeHTML(this._svg)}</span>`;
  }
}

if (!customElements.get("notatio-densityplot")) {
  customElements.define("notatio-densityplot", NotatioDensityPlot);
}
