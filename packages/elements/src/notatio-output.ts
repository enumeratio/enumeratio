...
import { collectErrors, deepEqual } from "./assert.ts";
...
/**
 * `<notatio-output>` -- read-only typeset rendering of a compute-engine
 ...
 */
export class NotatioOutput extends LitElement {
  static properties = {
  ...
  async #gpuShader(expr: { unknowns: ReadonlyArray<string> }): Promise<string | undefined> {
    ...
    try {
      ...
      const { computeShader, toWgslFn } = await import("./gpu-eval.ts");
...
if (!customElements.get("notatio-output")) {
  customElements.define("notatio-output", NotatioOutput);
}
