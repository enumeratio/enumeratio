...
import { gridFromPoints, mesh3dSvg, scatter3dSvg } from "./listplot3d.ts";
import type { Point3 } from "./project3d.ts";
...
/**
 * `<notatio-listplot3d data="[[0,1],[2,3]]">` -- 3-D data, orthographically
...
export class NotatioListPlot3d extends LitElement {
  ...
  protected override render(): unknown {
    return html`<span class="notatio-listplot3d-box">${unsafeHTML(this._svg)}</span>`;
  }
}

if (!customElements.get("notatio-listplot3d")) {
  customElements.define("notatio-listplot3d", NotatioListPlot3d);
}
