...
import { dirOf } from './pack-map'

const here = dirname(fileURLToPath(import.meta.url))
// dirOf tracks the file's actual placement mid-split (#283 phase 2.2 moved it to packs/refs/) — read from the
// map instead of hardcoding sqlsrc/, so this generator keeps writing to the right place as the split lands.
const OUT_FILE = join(here, dirOf('examples.render_corpus'), 'examples.render_corpus.sql')
...
const header = `-- requires: realizer, seed.render_corpus
