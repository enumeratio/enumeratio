...
-- The floor scans a generous bound keeping emirps ascending; contains is the same predicate. Unbounded ⇒ cardinality ∞.
...
CREATE FUNCTION fiber_elements(f emirp_primes_fiber, element_limit int) RETURNS SETOF numeric LANGUAGE sql STABLE AS $$
  SELECT n::numeric FROM generate_series(10, 20000) n WHERE is_emirp_prime(n::numeric) ORDER BY n LIMIT element_limit $$;
