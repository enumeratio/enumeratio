...
INSERT INTO base_example (suite, title, kind, expected, description, sql) VALUES
  ...
  ('capabilities','subsets is countable + samplable but NOT indexable (no direct unrank hook)','eq','countable:t indexable:f samplable:t','a finite collection whose floor scans',$q$
    SELECT string_agg(t || ':' || CASE WHEN EXISTS (SELECT 1 FROM base_collection_trait WHERE collection='subsets' AND trait=t) THEN 't' ELSE 'f' END, ' ' ORDER BY t)
