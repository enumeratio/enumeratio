...
INSERT INTO base_example (suite, title, kind, expected, description, sql) VALUES
  ('sampling','draw_cost heuristic as data: k_subsets O(1) (indexable) | factorial_numbers none (∞)','eq','factorial_numbers:none k_subsets:O(1)','the per-collection sampling cost, queryable not prose (partitions-plus adds a scan-floor case)',$q$
    SELECT string_agg(collection || ':' || draw_cost, ' ' ORDER BY collection)
    FROM base_collection_sampling WHERE collection IN ('k_subsets','factorial_numbers') $q$),
