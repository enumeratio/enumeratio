...
export const PACK_DEPS: Record<PackName, PackName[]> = {
  ...
  // edge-index engine (both real calls, not stale headers — grepped, and both already declared in their own
  // `-- requires:` headers): trees-graphs is extracted (#316), so this edge is now materialized — permutations-plus
  // loads after trees-graphs. orbit_maps_permutations_subsets reuses symmetry_orbit_maps' word_canonical_rotation
  // for its subset-necklace map — same shape as number-sets -> words-plus: words-plus isn't extracted yet, so
  // that file stays physically in core (sqlsrc/) and loads before permutations-plus regardless — this declares
  // the logical edge for pack-lint, ready for when words-plus extracts.
...
export const PACK_MAP: [PackName, RegExp][] = [
  ...
  ['number-sets', /^(.*_numbers|.*_primes|prime_gaps|collatz_trajectories|pythagorean_triples|sums_of_two_squares|goldbach_partitions|square_decompositions|egyptian_fractions|farey_sequences(\.maps)?|continued_fractions|calkin_wilf_paths|stern_brocot_paths|squarefree_semiprimes|k_almost_primes|k_free_integers|prime_pairs|zeckendorf_representations|hyperbinary_representations(\.stats)?|examples\.hypernumerary|number-gradings\.number-sets|generating_functions\.number-sets|fiber_unrank_verify\.number-sets|tags\.number-sets|examples\.representations\.number-sets|example-tiers\.number-sets|search_sequence\.number-sets)$/],
