...
export const PACK_MAP: [PackName, RegExp][] = [
  ...
  ['number-sets', /^(.*_numbers|.*_primes|prime_gaps|collatz_trajectories|pythagorean_triples|sums_of_two_squares|goldbach_partitions|square_decompositions|egyptian_fractions|farey_sequences(\.maps)?|continued_fractions|calkin_wilf_paths|stern_brocot_paths|squarefree_semiprimes|k_almost_primes|zeckendorf_representations|hyperbinary_representations(\.stats)?|examples\.hypernumerary|number-gradings\.number-sets|generating_functions\.number-sets|fiber_unrank_verify\.number-sets|tags\.number-sets|examples\.representations\.number-sets|example-tiers\.number-sets|search_sequence\.number-sets)$/],
