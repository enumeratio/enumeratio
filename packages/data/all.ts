// Browser/Vite entry for the FULL PROFILE — core + every extracted pack (#283 phase 4, wiki §7 / O.1-O.2). This
// is what index.ts (the root `@enumeratio/data` export) used to be, unconditionally, before the core/packs split
// narrowed the root export to core-only: docs and the explorer both need the whole catalog, so they import this
// entry (`@enumeratio/data/all`) explicitly instead. Deliberately a near-duplicate of index.ts rather than a
// config flag threaded through one shared file — each entry's import.meta.glob calls stay statically analyzable
// by Vite (a dynamic pattern would defeat the point of a build-time glob), and each entry ships exactly what it
// claims to, nothing more.
//
// Vite inlines the SQL at build via import.meta.glob(?raw); the node runner (run.mts) reads the same files from
...
// whether the dump is fresh (mount it) or stale (rebuild from sqlsrc). Must match node.ts's coreBundleHash('all').
export const coreBundleHash: string = bundleHash(coreBundle)

// Per-pack hashes (§7), one row per loaded pack. Lets the client detect exactly which pack(s) went stale in a
// mounted dump.
