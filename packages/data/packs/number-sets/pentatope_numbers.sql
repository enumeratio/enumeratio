-- requires: tetrahedral_numbers, realizer
...
INSERT INTO base_example (suite,title,kind,expected,description,sql) VALUES
  ...
  ('pentatope_numbers','P(n) - P(n-1) = Te(n) for n=1..8 (pentatope is the partial sums of tetrahedral)','eq','true','cross-check against the tetrahedral_numbers floor',$q$
    SELECT bool_and((unrank(pentatope_numbers(), n)).value - (unrank(pentatope_numbers(), n-1)).value = (unrank(tetrahedral_numbers(), n)).value)
