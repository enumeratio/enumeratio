 ...
 *
 * `elsewhere` is filled by `scripts/collect-coverage.ts`, which asks the kernels by NAME, so
 * a head spelled differently elsewhere lands here too. Most of this list is domain objects and
 * classical combinatorial families a general CAS doesn't carry; the exceptions are spellings:
 * ClausenCl (Wolfram's Im[PolyLog[n, E^(I θ)]], mpmath's clsin/clcos), RationalReconstruction
