...
import { provideDb, provideEngine, makeDb, standardEngine, close, terminalSelect } from '@enumeratio/client'
import { runCli } from '../src/dispatch.js'
import { nodeClient } from '../src/node-client.js'

// Drive the CLI's grammar (arg parsing → enumeratio-client → pglite) end to end, IN-PROCESS through the shared
// dispatch entry `runCli` — the exact function the binary (cli.ts) and the docs terminal both call — with a
// string-capturing writer over ONE pglite boot. This replaced ~1 subprocess spawn per assertion (each re-booting
// the core); a single binary smoke test below still exercises the real process (shebang, stdout, exit). A second,
// in-process db answers `terminalSelect` directly — the SAME registry function the CLI resolves against — so the
// header assertions check the printed columns against the policy, never a hardcoded copy of it (#246).
provideDb(() => makeDb())
...
// the ONE genuine subprocess: proves the built binary boots and prints over the real stdout path.
const cli = fileURLToPath(new URL('../src/cli.ts', import.meta.url))
const runProc = (args: string[]) => execFileSync('node', ['--import', 'tsx', cli, ...args], { encoding: 'utf8', timeout: 120_000 }).trim()

it('binary smoke: the real process lists the core collections over stdout', () => {
  const out = runProc(['list']).split('\n')
...
it('enumerates permutations(size=3) under its terminal-policy columns (#246)', async () => {
  ...
  const out = (await run(['permutations', 'size=3'])).split('\n')
...
it('a bare collection with no terminal-specific policy resolves its general one (integer_partitions, #246)', async () => {
  ...
  const lines = (await run(['integer_partitions', 'size=4'])).split('\n')
...
it('--count prints the cardinality', async () => {
  expect(await run(['integer_partitions', 'size=20', '--count'])).toBe('627')
})
...
it('a chain grade arg fixes a secondary axis', async () => {
  ...
  const out = (await run(['arrangements', 'size=5', 'length=2'])).split('\n')
...
it('a family-parameter range (name=LO:HI) enumerates the union of fibers', async () => {
  const cols = await terminalSelect('permutations')
  const out = (await run(['permutations', 'size=1:3'])).split('\n')
  expect(out[0]).toBe(cols.join('\t'))
  expect(out).toHaveLength(1 + 9) // header + 1! + 2! + 3! = 1 + 2 + 6
  expect(await run(['permutations', 'size=1:3', '--count'])).toBe('9')
})

it('--fibers lists the fibers a handle spans, with cardinalities', async () => {
  const out = (await run(['k_subsets', 'size=6', '--fibers'])).split('\n')
...
it('--group-by STAT prints a statistic distribution (the Mahonian row) with a summary', async () => {
  const out = (await run(['permutations', 'size=4', '-g', 'inversions'])).split('\n')
...
it('--triangle STAT prints the statistic distribution per fiber (the Mahonian triangle over sizes)', async () => {
  const out = (await run(['permutations', 'size=1:4', '--triangle', 'inversions'])).split('\n')
...
it('a restricted variant inherits its base carrier stats (carrier-level resolution)', async () => {
  ...
  const listed = await run(['list', 'compositions_into_k_parts'])
  expect(listed).toMatch(/stats\s+.*largest_part/)
  const stats = (await run(['compositions_into_k_parts', 'n=4', 'k=2', '--range', '0', '--fields', 'largest_part'])).split('\n')
  ...
  expect(await run(['list', 'derangements'])).toMatch(/reprs\s+.*cycle/)
  expect(await run(['derangements', 'size=4', '-R', 'cycle', '--range', '0'])).toBe('(1 2)(3 4)')
})

it('--at inspects one element: its rank, every statistic, and every map image', async () => {
  const byRank = (await run(['permutations', 'size=4', '--at', '9'])).split('\n')
  ...
  const bySer = (await run(['permutations', 'size=4', '--at', '@2413'])).split('\n')
  expect(bySer[0]).toBe('element  2413  (rank 10)')
  expect(bySer.find((l) => l.startsWith('maps'))).toMatch(/rsk_insertion=1,3\/2,4/)
})

it('maps <collection> lists the map graph edges out of a collection', async () => {
  const out = await run(['maps', 'permutations'])
...
it('--through composes maps across collections (permutation → RSK P tableau → its shape)', async () => {
  const out = (await run(['permutations', 'size=3', '--through', 'rsk_insertion,shape'])).split('\n')
...
it('-m projects each named map image as its own column', async () => {
  const out = (await run(['permutations', 'size=3', '-m', 'to_lehmer_code,descent_set'])).split('\n')
...
it('--range slices the canonical order', async () => {
  ...
  const out = (await run(['permutations', 'size=5', '--range', '0:3'])).split('\n')
  expect(out[0]).toBe(cols.join('\t'))
  expect(out.slice(1).map((l) => l.split('\t')[idx])).toEqual(['12345', '12354', '12435']) // undelimited one-line words
})

it('list <collection> shows the catalog shape', async () => {
  const out = await run(['list', 'set_partitions'])
  expect(out).toContain('set_partitions')
...
it('--repr serializes elements in an alternate representation', async () => {
  // permutations(3) rank 0 = identity → cycle notation lists every fixed point
  const cyc = (await run(['permutations', 'size=3', '--repr', 'cycle', '--range', '0'])).trim()
  expect(cyc).toBe('(1)(2)(3)')
  // set_partitions' blocks repr renders the RGS word as brace/slash blocks
  const blocks = (await run(['set_partitions', 'size=3', '--repr', 'blocks', '--range', '0:2'])).split('\n')
