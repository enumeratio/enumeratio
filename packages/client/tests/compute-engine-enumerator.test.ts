...
// Pascal-head cases (Permutations, DyckPaths, KSubsets, …) resolve via compute-engine-enumerator's own LIBRARY_TWIN
...
const snapshot: CatalogSnapshot = {
  ...
  typeOperations: [],
  collections: [
    collRow('permutations', { head: 'Permutations', arity: 1 }),
...
describe('computeEngineEnumerator — pure-CE enumeration over library CollectionHandlers', () => {
  it('unrank = At(coll, r+1), 0-based in / 1-based out', async () => {
    expect(await one(ap('unrank', handle('Permutations', 4), lit(0)))).toBe('[1,2,3,4]') // identity
    expect(await one(ap('unrank', handle('Permutations', 4), lit(23)))).toBe('[4,3,2,1]') // last, 24=4!
  })
  ...
  it('rank ∘ unrank = id (0-based round-trip through CE)', async () => {
    expect(await one(ap('rank', ap('unrank', handle('Permutations', 4), lit(5))))).toBe('5')
  })

  it('locate: rank(locate(handle, v)) is the 0-based rank of a member, empty for a non-member', async () => {
    expect(await one(ap('rank', ap('locate', handle('Permutations', 4), lit([1, 3, 2, 4]))))).toBe('2')
    expect(await one(ap('rank', ap('locate', handle('Permutations', 4), lit([1, 1, 2, 4]))))).toBe('') // not a member
  })

  it('cast(locate(handle, v)) re-emits the located value itself', async () => {
    const col: SelectExpr = { kind: 'cast', expr: ap('locate', handle('Permutations', 4), lit([1, 3, 2, 4])), to: 'permutation' }
    expect(await one(col)).toBe('[1,3,2,4]')
  })

  it('next/prev step the rank', async () => {
    expect(await one(ap('next', ap('unrank', handle('Permutations', 4), lit(0))))).toBe('[1,2,4,3]') // rank 1
    expect(await one(ap('prev', ap('unrank', handle('Permutations', 4), lit(23))))).toBe('[4,3,1,2]') // rank 22
  })
  ...
  it('can(): claims enumeration trees, declines FROM row-queries, pure scalars, and untwinned collections', () => {
    expect(engine.can({ select: [ap('unrank', handle('Permutations', 4), lit(0))] })).toBe(true)
    ...
    expect(engine.can({ select: [handle('Permutations', 4)], from: { from: { coll: 'x', named: {}, positional: [] } } as any })).toBe(false)
  })
})
