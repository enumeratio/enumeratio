...
const conjugate: MathJSON = forEach(
  upTo(largest),
  count(positions, atLeastValue(at("j"), "i"), "j"),
);
...
const hookProduct: MathJSON = [
  "Product",
  forEach(positions, ["Product", forEach(upTo(here), hookAt, "j")]),
];
...
export const PARTITION_STATISTICS: readonly Definition[] = [
  ...
  stat(
    "EvenParts",
    "Parts that are even.",
    nonEmpty(count(positions, equals(["Mod", here, 2], 0))),
  ),
  ...
  stat(
    "PartsAtLeastTwo",
    "Parts of size at least 2.",
    nonEmpty(count(positions, atLeastValue(here, 2))),
  ),
  ...
  stat(
    "LegOfFirstCell",
    "The leg of cell (1,1): the number of parts minus one.",
    nonEmpty(legAt(1)),
  ),
  ...
  stat(
    ...
    nonEmpty([
      ...
      equals(ones, 0),
      largest,
      subtract(count(positions, greater(here, ones)), ones),
    ]),
  ),
];
