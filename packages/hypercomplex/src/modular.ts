...
/**
 * Wolfram's `PowerModList[a, 1/r, m]`: every x in [0, m) with xʳ ≡ a (mod m), ascending.
 * One general head instead of a bespoke one per target — the split units of ℤ/m are
 * `PowerModList(1, 1/2, m)` and its imaginary units are `PowerModList(-1, 1/2, m)`.
 *
 * Square roots of ±1 take the exact CRT route above, which is unbounded in m and carries
 * the Hensel lift. Everything else is a scan of [0, m), so it is capped: `undefined`
 * above `BRUTE_FORCE_LIMIT` rather than a wrong or very slow answer.
 */
export const BRUTE_FORCE_LIMIT = 1_000_000;

export function powerModList(a: number, root: number, m: number): number[] | undefined {
  if (!Number.isInteger(a) || !Number.isInteger(root) || !Number.isInteger(m)) return undefined;
  if (root < 1 || m < 1) return undefined;
  const target = ((a % m) + m) % m;
  if (root === 2 && (target === 1 % m || target === ((-1 % m) + m) % m)) {
    // The exact path: CRT channels, with √−1 Hensel-lifted per prime power.
    const exact = target === 1 % m ? splitUnitsMod(m) : imaginaryUnitsMod(m);
    // For m ≤ 2 the two targets coincide; splitUnitsMod already answers both.
    if (m > 2 || target === 1 % m) return exact;
  }
  if (m > BRUTE_FORCE_LIMIT) return undefined;
  const found: number[] = [];
  for (let x = 0; x < m; x++) {
    let power = 1;
    for (let k = 0; k < root; k++) power = mulMod(power, x, m);
    if (power === target) found.push(x);
  }
  return found;
}

/** `PowerModList[a, k, m]` for a non-negative integer k — the single value aᵏ mod m. */
export function powerMod(a: number, exponent: number, m: number): number | undefined {
  if (!Number.isInteger(a) || !Number.isInteger(exponent) || exponent < 0 || m < 1)
    return undefined;
  let result = 1 % m;
  let base = ((a % m) + m) % m;
  for (let e = exponent; e > 0; e >>= 1) {
    if (e % 2 === 1) result = mulMod(result, base, m);
