...
const props = defineProps<{ collection: string; n: number }>()
...
const needsGround = computed(() => isModular.value || isFinset.value)   // both take the collection's n (modulus / ground [n])
const ringLabel = computed(() => (isModular.value ? `ℤ/${Math.trunc(props.n)}ℤ` : isFinset.value ? `𝒫([${Math.trunc(props.n)}])` : carrier.value))
...
// Modular expression examples are authored pinned to m = 5 (see expression_examples.sql header); their expected
// values only hold in ℤ/5ℤ. So only assert a green/red verdict when the page's modulus IS 5 — at any other ℤ/nℤ the
// calculator still evaluates live (correctly for that n), it just doesn't check against the m=5 expected value.
const EXAMPLE_MODULUS = 5
const verdict = computed<null | 'ok' | 'bad'>(() => {
  if (!matched.value || error.value || result.value == null) return null
  if (isModular.value && Math.trunc(props.n) !== EXAMPLE_MODULUS) return null
...
async function run() {
  ...
  const r = await evaluateExpression(carrier.value, expr.value, needsGround.value ? Math.trunc(props.n) : undefined)
...
watch([expr, () => props.n, carrier], run)
</script>
