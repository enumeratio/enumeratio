...
var script$9 = {
  ...
  computed: {
    ...
    shouldRenderRowGroupHeader: function shouldRenderRowGroupHeader() {
      ...
      var prevRowData = this.value[this.index - 1];
      if (prevRowData) {
    ...
    shouldRenderRowGroupFooter: function shouldRenderRowGroupFooter() {
      if (this.expandableRowGroups && !this.isRowGroupExpanded) {
        ...
        var currentRowFieldData = resolveFieldData(this.rowData, this.groupRowsBy);
        var nextRowData = this.value[this.index + 1];
        if (nextRowData) {
...
var script = {
  ...
  methods: {
    ...
    updateRowGroupHeaderHeight: function updateRowGroupHeaderHeight() {
      if (this.rowGroupMode !== 'subheader' || !this.groupRowsBy || this.virtualScrollerDisabled) {
        return;
      }
      var header = this.$refs.table && findSingle(this.$refs.table, 'tr.p-datatable-row-group-header');
      if (header) {
        var height = getOuterHeight(header);
        if (height && height !== this.d_rowGroupHeaderHeight) {
          this.d_rowGroupHeaderHeight = height;
        }
      }
    },
    getRenderedWindowSize: function getRenderedWindowSize(slotProps) {
      if (!slotProps || !slotProps.rows || !slotProps.rows.length) {
        return 0;
      }
      var fn = this.getItemSize;
      if (typeof fn !== 'function') {
        return slotProps.rows.length * slotProps.itemSize;
      }
      var total = 0;
      for (var i = 0; i < slotProps.rows.length; i++) {
  ...
  computed: {
    ...
    getItemSize: function getItemSize() {
      var baseSize = this.virtualScrollerOptions && this.virtualScrollerOptions.itemSize || 0;
      var groupRowsBy = this.groupRowsBy;
      var subheader = this.rowGroupMode === 'subheader' && !!groupRowsBy;
      if (!subheader) {
        return null;
      }
      var items = this.processedData;
      var expandable = this.expandableRowGroups;
      var expandedGroups = this.expandedRowGroups;
      var headerHeight = this.d_rowGroupHeaderHeight;
      return function (index) {
        if (!items || index < 0 || index >= items.length) {
          return baseSize;
        }
        var groupValue = resolveFieldData(items[index], groupRowsBy);
        var isFirstOfGroup = index === 0 || resolveFieldData(items[index - 1], groupRowsBy) !== groupValue;
        var isExpanded = !expandable || expandedGroups && expandedGroups.indexOf(groupValue) > -1;
  ...
  return openBlock(), createElementBlock("div", mergeProps({
    ...
    content: withCtx(function (slotProps) {
      return [createElementVNode("table", mergeProps({
        ...
        style: {
          height: "calc(".concat(slotProps.spacerStyle.height, " - ").concat($options.getRenderedWindowSize(slotProps), "px)")
        }
    ...
    _: 1
  }, 16, ["items", "columns", "style", "scrollHeight", "disabled", "getItemSize", "pt"])], 16, _hoisted_2), $options.paginatorBottom ? (openBlock(), createBlock(_component_DTPaginator, {
    key: 2,
    rows: $data.d_rows,
    first: $data.d_first,
    totalRecords: $options.totalRecordsLength,
