...
var script = {
  ...
  methods: {
    ...
    getItemOffset: function getItemOffset(index) {
      if (this.cumulativeSizes) {
        var i = Math.max(0, Math.min(index, this.cumulativeSizes.length - 1));
        return this.cumulativeSizes[i];
      }
      return index * this.itemSize;
    },
    getIndexAtOffset: function getIndexAtOffset(offset) {
      if (!this.cumulativeSizes) {
        return Math.floor(offset / (this.itemSize || offset));
      }
      var sizes = this.cumulativeSizes;
      ...
      while (lo < hi) {
        var mid = lo + hi >>> 1;
        if (sizes[mid + 1] <= offset) lo = mid + 1;else hi = mid;
      }
      return lo;
    },
    getLastByCumulative: function getLastByCumulative(first, numToleratedItems) {
      if (!this.cumulativeSizes) return this.getLast(first);
      var sizes = this.cumulativeSizes;
      var contentPos = this.getContentPosition();
      var contentHeight = this.element ? this.element.offsetHeight - contentPos.top : 0;
      var startOffset = sizes[Math.min(first, sizes.length - 1)];
      var target = startOffset + contentHeight + 2 * numToleratedItems * this.itemSize;
      var lo = first;
      var hi = sizes.length - 1;
      while (lo < hi) {
        var mid = lo + hi >>> 1;
        if (sizes[mid] < target) lo = mid + 1;else hi = mid;
      }
      return lo;
    },
          ...
          if (isToStart) {
            if (both) {
              ...
              if (viewport.first - first > index) {
                var target = viewport.first - 1;
                var pos = !horizontal && this.cumulativeSizes ? this.getItemOffset(target) : target * this.itemSize;
            ...
            if (both) {
              ...
              if (viewport.last - first <= index + 1) {
                var _target = viewport.first + 1;
                var _pos2 = !horizontal && this.cumulativeSizes ? this.getItemOffset(_target) : _target * this.itemSize;
    ...
    setSpacerSize: function setSpacerSize() {
      ...
      if (items) {
        ...
        var setVerticalSize = function setVerticalSize() {
          var _cpos = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
          return _this6.spacerStyle = _objectSpread(_objectSpread({}, _this6.spacerStyle), {}, {
            height: _this6.cumulativeSizes[_this6.cumulativeSizes.length - 1] + _cpos + 'px'
          });
        };
        if (both) {
    ...
    onScrollPositionChange: function onScrollPositionChange(event) {
      ...
      if (both) {
        ...
        if (!this.appendOnly || this.appendOnly && isScrollDownOrRight) {
          var _currentIndex2 = !horizontal && this.cumulativeSizes ? this.getIndexAtOffset(scrollPos) : calculateCurrentIndex(scrollPos, this.itemSize);
          var _triggerIndex2 = calculateTriggerIndex(_currentIndex2, this.first, this.last, this.numItemsInViewport, this.d_numToleratedItems, isScrollDownOrRight);
          newFirst = calculateFirst(_currentIndex2, _triggerIndex2, this.first, this.last, this.numItemsInViewport, this.d_numToleratedItems, isScrollDownOrRight);
          newLast = !horizontal && this.cumulativeSizes ? this.getLastByCumulative(newFirst, this.d_numToleratedItems) : calculateLast(_currentIndex2, newFirst, this.last, this.numItemsInViewport, this.d_numToleratedItems);
