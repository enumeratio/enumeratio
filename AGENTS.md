...
- **No attribution.** Never add "Generated with Claude Code", "Generated by Claude Code",
  session links, or `Co-Authored-By`/`Claude-Session` trailers to commits, PR descriptions,
  or comments. `.claude/settings.json` turns off the harness's own; this covers the rest.
